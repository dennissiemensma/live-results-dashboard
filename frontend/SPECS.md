...
- [x] "Final lap" (blue) badge: rendered immediately to the right of the lap counter inside the centre column (i.e. between the lap counter and the right column that shows gap/total time); finished competitor rows are slightly opaque; finished state is indicated by a faint green diagonal **"Finished"** watermark on the row — no "Finished" badge anywhere
...
- [x] Laps badge: **mass start only** — "X/total" with total in small gray; fixed min-width for "XX/XX" (5 chars + padding), centered, monospace, rendered inline after the time/gap field; **timed distance**: no laps badge and no "Finished" badge — finished state shown via watermark only
