...
- [x] Hide all mass start settings in the top bar when there is no mass start in any of the distances
...
- [x] Top bar layout (left to right): event name → connection status badge → **"Show server updates"** button → (ms-auto) **"Mass Start Settings"** label + vertical separator → UI settings (Max seconds gap, Max groups, Lap Δ, Lap times toggle)
...
- [x] Laps badge: **mass start only** — "X/total" with total in small gray; fixed min-width for "XX/XX" (5 chars + padding), centered, monospace, rendered inline after the time/gap field; **timed distance**: no laps badge, no "Finished" badge, and no finished watermark — finished state is not visually annotated beyond the text dimming that applies to all finished rows
